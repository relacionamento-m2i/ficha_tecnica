...
# ==========================================
# 1. Traz o banco de dados para a memória persistente
...
if 'servico_atual' not in st.session_state:
    ...
    st.session_state['servico_atual'] = primeiro_servico
    st.session_state['tempo_min'] = dados_iniciais['tempo_min']
    st.session_state['repasse_fixo'] = dados_iniciais['repasse_fixo']
    st.session_state['df_ficha_maquinas'] = pd.DataFrame(dados_iniciais['maquinas']) if dados_iniciais['maquinas'] else pd.DataFrame({"nome": [""], "custo": [0.0]})
    st.session_state['df_ficha_insumos'] = pd.DataFrame(dados_iniciais['insumos']) if dados_iniciais['insumos'] else pd.DataFrame({"Material": [""], "QT": [0], "Preço (R$)": [0.0]})
    st.session_state['taxa_comissao'] = float(dados_iniciais['taxas']['comissao'])
    st.session_state['taxa_cartao'] = float(dados_iniciais['taxas']['cartao'])
    st.session_state['taxa_imposto'] = float(dados_iniciais['taxas']['imposto'])
    st.session_state['preco_escolhido'] = float(dados_iniciais['preco_escolhido'])
    ...
    with col_save:
        if st.button("💾 Atualizar Serviço no Banco"):
            st.session_state['db_servicos'][st.session_state['servico_atual']] = {
                ...
                "taxas": {
                    "comissao": st.session_state['taxa_comissao'] * 100, 
                    "cartao": st.session_state['taxa_cartao'] * 100, 
                    "imposto": st.session_state['taxa_imposto'] * 100,
                    "repasse_liq": 0.0,
                    "lucro": 0.0
                },
                "preco_escolhido": st.session_state['preco_escolhido']
...
# ==========================================
# MÓDULO 2: CUSTOS FIXOS (ATUALIZADO COM TABELAS DINÂMICAS)
# ==========================================
def render_custos_fixos():
    
    # Inicializa os dataframes das categorias se não existirem na memória
    ...
    # Função para renderizar a tabela dinâmica de cada categoria
    def renderizar_categoria_dinamica(titulo, chave):
        with st.expander(titulo, expanded=False):
            df_atual = st.session_state["df_custos_categorias"][titulo]
            
            # st.data_editor permite adicionar e remover linhas dinamicamente
            ...
            # Cálculo seguro do total, ignorando se o usuário deixar uma célula vazia
    ...
    with st.sidebar:
        ...
        if arquivo_upload is not None:
            try:
                ...
                # Carrega os parâmetros de dias/horas
        ...
        # Estrutura os dados atuais para salvar
        dados_para_salvar = {
