...
# ==========================================
# Carrega as senhas do arquivo seguro .streamlit/secrets.toml
try:
    CREDENCIAIS = st.secrets["credenciais"]
except Exception:
    st.error("⚠️ Arquivo de senhas (secrets.toml) não encontrado ou configurado incorretamente. O sistema não pode ser iniciado.")
    st.info("Crie a pasta `.streamlit` e o arquivo `secrets.toml` contendo a categoria `[credenciais]` para continuar.")
    st.stop()
...
def tela_login():
    st.markdown(f"""
    <div style="background-color: {COR_CABECALHO}; padding: 20px; border-radius: 8px; text-align: center; margin-bottom: 30px;">
        <h1 style="margin: 0; color: {COR_TEXTO_BRANCO};">🔒 Acesso Restrito</h1>
        <p style="color: {COR_TEXTO_BRANCO};">Por favor, faça login para acessar o Sistema de Precificação</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        with st.form("form_login"):
            ...
            if submit:
                if usuario in CREDENCIAIS and str(CREDENCIAIS[usuario]) == str(senha):
...
# Bloqueia a visualização do resto se não estiver logado
...
if "df_lista_insumos" not in st.session_state:
    st.session_state["df_lista_insumos"] = pd.DataFrame({"Material": ["AGULHA", "SERINGA"], "qt": [1.0, 1.0], "valor": [0.56, 1.20]})
...
with st.sidebar:
    ...
    st.write(f"👤 Logado como: **{st.session_state.get('usuario_logado', '')}**")
...
def render_ficha_tecnica():
    ...
    with col_esq:
        ...
        if regs_maq:
            for i, reg in enumerate(regs_maq):
                ...
                novo_nome_maq = c1.text_input("Máquina", value=reg['nome'], key=f"edit_nome_maq_{i}", label_visibility="collapsed")
                novo_custo_maq = c2.number_input("Custo", value=float(reg['custo']), min_value=0.0, step=10.0, format="%.2f", key=f"edit_custo_maq_{i}", label_visibility="collapsed")
                
                regs_maq[i]['nome'] = novo_nome_maq
                regs_maq[i]['custo'] = novo_custo_maq
        ...
        # Integração Dinâmica com Registro de Equipamentos
        df_eq = st.session_state.get("df_lista_equipamentos", pd.DataFrame())
        opcoes_eq = ["-- Digitar Manualmente --"] + df_eq["Nome do equipamento"].tolist() if not df_eq.empty else ["-- Digitar Manualmente --"]
        
        st.write("➕ **Adicionar Máquina ao Serviço**")
        sel_eq = st.selectbox("Buscar Equipamento Cadastrado:", opcoes_eq, key="sel_add_maq")
        
        default_nome_eq = sel_eq if sel_eq != "-- Digitar Manualmente --" else ""
        default_custo_eq = 0.0
        
        if sel_eq != "-- Digitar Manualmente --":
            row = df_eq[df_eq["Nome do equipamento"] == sel_eq].iloc[0]
        ...
        if regs_ins:
            for i, reg in enumerate(regs_ins):
                ...
                novo_mat = c1.text_input("Material", value=reg['Material'], key=f"edit_mat_{i}", label_visibility="collapsed")
                novo_qt = c2.number_input("Qtd", value=float(reg['QT']), min_value=0.01, step=1.0, format="%.2f", key=f"edit_qt_ins_{i}", label_visibility="collapsed")
                novo_preco = c3.number_input("Preço Un.", value=float(reg['Preço (R$)']), min_value=0.0, step=0.10, format="%.3f", key=f"edit_preco_ins_{i}", label_visibility="collapsed")
                
                regs_ins[i]['Material'] = novo_mat
                regs_ins[i]['QT'] = novo_qt
                regs_ins[i]['Preço (R$)'] = novo_preco
        ...
        # Integração Dinâmica com Insumos e Materiais
        ...
        sel_ins = st.selectbox("Buscar Insumo Cadastrado:", opcoes_ins, key="sel_add_ins")
    ...
    # Callback para puxar a Taxa de Cartão globalmente
