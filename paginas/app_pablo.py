...
if "df_lista_equipamentos" not in st.session_state:
    st.session_state["df_lista_equipamentos"] = pd.DataFrame({
        "Nome do equipamento": [
            "ULTRASSOM", "ONDA DE CHOQUE", "LASER", "SIS", "CAPACETE DE LED"
        ],
        "Valor de aquisição (R$)": [
            109000.00, 166000.00, 250000.00, 104562.74, 10000.00
        ], 
        "Tempo de vida útil (anos)": [
            10.0, 5.0, 10.0, 10.0, 10.0
        ],
        "Capacidade de Aplicações / dia (R$)": [
            16.0, 16.0, 16.0, 16.0, 20.0
        ], 
        "Aplicações (média diária)": [
            8.0, 8.0, 8.0, 8.0, 10.0
        ], 
        "Custo anual de manutenção (R$)": [
            0.0, 2900.00, 2000.00, 2000.00, 0.0
        ]
    })

if "df_lista_insumos" not in st.session_state:
    lista_insumos_padrao = [
        ("Algodão", 1.0, 28.28), ("Kit MMP 1 sessão", 6.0, 300.00), ("PRP", 1.0, 3.00),
        ("Cartucho MMP", 1.0, 30.00), ("Exossomo", 1.0, 600.00), ("Insumo exossomo (cartucho +med)", 1.0, 30.00),
        ("Agulha 18G", 1.0, 0.18), ("Agulha 26G", 1.0, 0.14), ("Agulha 30G", 1.0, 0.18),
        ("Cateter jelco 22G", 1.0, 1.44), ("Cateter jelco 20G", 1.0, 1.00), ("Clorexidina alcóolica", 1.0, 13.20),
        ("Clorexidina aquosa", 1.0, 17.75), ("Compressa estéril 25x28", 1.0, 6.79), ("Compressa não estéril 45x50", 50.0, 96.37),
        ("Equipo de soro", 1.0, 0.06), ("Esparadrapo", 1.0, 0.05), ("Gaze estéril 7,5 cm x 7,5 cm", 1.0, 0.08),
        ("Luva estéril 7,0", 1.0, 4.77), ("Luva estéril 7,5", 1.0, 4.77), ("Micropori", 1.0, 17.34),
        ("Rolo de papel filme grande", 3.0, 49.00), ("Seringa com luer lock 20 ml", 1.0, 1.18), ("Seringa com luer lock 3 ml", 1.0, 0.31),
        ("Seringa com luer lock 5 ml", 1.0, 0.25), ("Seringa com luer lock10 ml", 1.0, 1.18), ("Tapete Higiênico (Tipo Pet – Utilizado na absorção sanguínea na ultima fase cirúrgica)", 1.0, 2.81),
        ("Transofix", 1.0, 1.00), ("Azul de metileno", 3.0, 60.00), ("Bromoprida 10 mg IV", 1.0, 2.40),
        ("Cefazolina 2g IV ou Cefalotina 1g IV", 1.0, 6.70), ("Ciclobenzaprina 10 mg VO", 1.0, 0.90), ("Clonidina IV", 3.0, 26.85),
        ("Decadron 10 mg IV", 9.0, 73.78), ("Dipirona 1g IV", 1.0, 1.15), ("Midazolam 7,5 mg VO", 1.0, 7.91),
        ("Ondansetrona 4 mg IV", 1.0, 2.33), ("Pantoprazol 40 mg VO", 1.0, 12.22), ("Transamin 50mg/ml IV", 1.0, 5.29),
        ("Adrenalina 1 ml", 8.0, 8.40), ("Água p/injeção", 1.0, 0.49), ("Lidocaína 2% sem vasoconstritor 20 ml", 1.0, 12.42),
        ("Novabupi 0,5% sem vasoconstritor 20 ml", 1.0, 42.41), ("SORO FISIOLÓGIGO 500 ml", 1.0, 9.23), ("SORO FISIOLÓGICO 250 ml", 1.0, 5.38),
        ("SORO GLICOSADO 250 ml", 1.0, 4.80), ("Avental descartável com manga", 1.0, 4.40), ("Campo cirúrgico G", 1.0, 6.79),
        ("Campo Cirurgico M", 1.0, 1.92), ("Capa para vídeo estéril", 1.0, 4.82), ("Gelox", 1.0, 5.75),
        ("Luva de procedimento M", 1.0, 0.28), ("Luva de procedimento P", 1.0, 0.28), ("Máscara", 1.0, 0.50),
        ("Touca", 1.0, 0.15), ("Shampoo (7ml)", 1.0, 24.99), ("Óleo", 1.0, 56.99),
        ("Punch", 1.0, 230.00), ("Lâminas Transplante", 1.0, 25.00), ("Agulha do implanter", 1.0, 100.00),
        ("Kit cirúrgico estável médico", 5.0, 160.00), ("Necessarie do Paciente", 1.0, 29.87), ("Almofada", 1.0, 43.78),
        ("Pente", 1.0, 0.60), ("Óleo Paciente", 1.0, 5.70), ("Frascos", 1.0, 20.79),
        ("Shampoo Paciente", 1.0, 12.79)
    ]
...
if "db_funcionarios" not in st.session_state:
        ...
        with st.expander(titulo, expanded=False):
            ...
            if registros:
                for idx, reg in enumerate(registros):
                    ...
                    # Travar edição manual da linha automatizada do RH
                    se_automatizado = (novo_item == "1.1 Total da folha de pagamento")
                    
                    novo_valor = c2.number_input("Valor", value=float(reg['MENSAL (R$)']), min_value=0.0, step=10.0, format="%.2f", key=f"edit_val_{chave}_{idx}", label_visibility="collapsed", disabled=se_automatizado)
                    
                    registros[idx]['ÍTEM'] = novo_item
                    registros[idx]['MENSAL (R$)'] = novo_valor
                    
                    if c3.button("🗑️", key=f"del_{chave}_{idx}") and not se_automatizado:
                        registros.pop(idx)
...
# ==========================================
# MÓDULO 6: GESTÃO DE RH E INTEGRAÇÃO (ATUALIZADO COM FUNÇÃO)
# ==========================================
def render_calculadora_rh():
    with st.sidebar:
        st.header("💾 Salvar / Carregar RH")
        arquivo_upload = st.file_uploader("Carregar backup (.json)", type=["json"], key="up_rh")
        if arquivo_upload is not None:
            try:
                st.session_state["db_funcionarios"] = json.load(arquivo_upload)
                st.success("Dados de RH carregados!")
            except Exception: st.error("Erro ao ler o arquivo.")
        st.divider()
        st.download_button("📥 Baixar Cenário de RH", data=json.dumps(st.session_state["db_funcionarios"], indent=4), file_name="rh_funcionarios.json", mime="application/json")

    st.title("Gestão de Quadro de Funcionários (RH)")
    st.markdown("Cadastre sua equipe separada por função para descobrir o **custo real mensal**. O sistema integrará automaticamente a soma total no módulo de Custos Fixos.")
    st.divider()

    tab_cadastro, tab_lista = st.tabs(["➕ Cadastrar / Editar Funcionário", "📋 Quadro Geral e Filtros"])

    with tab_cadastro:
        st.markdown("💡 *Se você digitar o nome de um funcionário que já existe, os dados dele serão atualizados.*")
        
        c_nome, c_funcao = st.columns(2)
        nome_func = c_nome.text_input("Nome do Funcionário", placeholder="Ex: Ana Silva")
        funcao_func = c_funcao.text_input("Função / Cargo", placeholder="Ex: Recepcionista")

        st.subheader("1. Remuneração Mensal")
        c1, c2, c3 = st.columns(3)
        salario_base = c1.number_input("Salário Base (R$)", min_value=0.0, step=100.0, value=2000.0)
        comissoes = c2.number_input("Média de Comissões (R$)", min_value=0.0, step=50.0, value=0.0)
        horas_extras = c3.number_input("Média de Horas Extras (R$)", min_value=0.0, step=50.0, value=0.0)

        remuneracao_total = salario_base + comissoes + horas_extras

        st.subheader("2. Benefícios (Custo assumido pela Clínica)")
        c4, c5, c6 = st.columns(3)
        vale_transporte = c4.number_input("Vale Transporte (R$)", min_value=0.0, step=10.0, value=0.0)
        vale_alimentacao = c5.number_input("Vale Refeição / Alimentação (R$)", min_value=0.0, step=10.0, value=0.0)
        plano_saude = c6.number_input("Plano de Saúde / Outros (R$)", min_value=0.0, step=10.0, value=0.0)

        beneficios_total = vale_transporte + vale_alimentacao + plano_saude

        st.subheader("3. Encargos Sociais sobre a Folha (%)")
        encargos_pct = st.number_input("Alíquota Total de Encargos do Empregador (%)", min_value=0.0, max_value=100.0, step=1.0, value=8.0)

        st.write("")
        if st.button("💾 Salvar Funcionário e Sincronizar com Custos Fixos", type="primary"):
            if not nome_func.strip():
                st.warning("Por favor, digite o nome do funcionário antes de salvar.")
            else:
                provisao_13 = remuneracao_total / 12
                provisao_ferias = (remuneracao_total / 12) + (remuneracao_total / 36) 
                base_encargos = remuneracao_total + provisao_13 + provisao_ferias
                valor_encargos = base_encargos * (encargos_pct / 100)

                custo_mensal = remuneracao_total + provisao_13 + provisao_ferias + valor_encargos + beneficios_total
                custo_anual = custo_mensal * 12

                # SALVANDO JUNTO COM A FUNÇÃO
                st.session_state["db_funcionarios"][nome_func] = {
                    "funcao": funcao_func.strip() if funcao_func.strip() else "Sem Função",
                    "remuneracao_total": remuneracao_total,
                    "provisoes_mensais": provisao_13 + provisao_ferias,
                ...
                # INTEGRAÇÃO COM CUSTOS FIXOS (TOTAL GERAL)
                total_folha = sum(f["custo_mensal"] for f in st.session_state["db_funcionarios"].values())
                
                df_pessoal = st.session_state["df_custos_categorias"]["1. Despesa com pessoal"].copy()
                if "1.1 Total da folha de pagamento" in df_pessoal["ÍTEM"].values:
                    idx = df_pessoal.index[df_pessoal["ÍTEM"] == "1.1 Total da folha de pagamento"].tolist()[0]
                    df_pessoal.loc[idx, "MENSAL (R$)"] = total_folha
                else:
                    novo_item = pd.DataFrame([{"ÍTEM": "1.1 Total da folha de pagamento", "MENSAL (R$)": total_folha}])
                    df_pessoal = pd.concat([df_pessoal, novo_item], ignore_index=True)
                
                st.session_state["df_custos_categorias"]["1. Despesa com pessoal"] = df_pessoal

                st.success(f"✅ Funcionário '{nome_func}' salvo! Valor atualizado na aba de Custos Fixos.")

    with tab_lista:
        if st.session_state["db_funcionarios"]:
            # Filtro Dinâmico por Função
            todas_funcoes = sorted(list(set(v.get("funcao", "Sem Função") for v in st.session_state["db_funcionarios"].values())))
            
            c_filtro, c_vazio2 = st.columns([1, 2])
            filtro_funcao = c_filtro.selectbox("🔎 Filtrar tabela por Função / Cargo:", ["-- Mostrar Todos --"] + todas_funcoes)
            
            # Montando dados filtrados
            dados_tabela = []
            for k, v in st.session_state["db_funcionarios"].items():
                func_atual = v.get("funcao", "Sem Função")
                if filtro_funcao == "-- Mostrar Todos --" or func_atual == filtro_funcao:
                    dados_tabela.append({
                        "Nome do Funcionário": k,
                        "Função": func_atual,
                        "Remuneração Direta": v["remuneracao_total"], 
                        "Provisões (Férias/13º)": v["provisoes_mensais"], 
                        "Encargos Sociais": v["valor_encargos"], 
                        "Benefícios": v["beneficios_total"], 
                        "CUSTO MENSAL": v["custo_mensal"], 
                        "CUSTO ANUAL": v["custo_anual"]
                    })
            
            if dados_tabela:
                df_funcs = pd.DataFrame(dados_tabela)
                st.dataframe(df_funcs.style.format({c: "R$ {:,.2f}" for c in df_funcs.columns if c not in ["Nome do Funcionário", "Função"]}).apply(lambda x: ["background: #E6E0EC; font-weight: bold" if x.name in ["CUSTO MENSAL", "CUSTO ANUAL"] else "" for _ in x], axis=0), use_container_width=True, hide_index=True)
                
                total_mensal = df_funcs["CUSTO MENSAL"].sum()
                total_anual = df_funcs["CUSTO ANUAL"].sum()
                
                c_tot1, c_tot2 = st.columns(2)
                c_tot1.metric(f"Impacto Mensal ({filtro_funcao})", f"R$ {total_mensal:,.2f}")
                c_tot2.metric(f"Impacto Anual ({filtro_funcao})", f"R$ {total_anual:,.2f}")
            else:
            ...
            # Gráfico 1: Distribuição por Funcionário (Baseado no filtro ativo)
            if dados_tabela:
                fig1 = px.pie(df_funcs, values='CUSTO MENSAL', names='Nome do Funcionário', hole=0.4, title="Custo por Funcionário (Filtrado)")
                fig1.update_traces(textposition='inside', textinfo='percent+label')
                fig1.update_layout(showlegend=False)
                c_graf1.plotly_chart(fig1, use_container_width=True)

            # Gráfico 2: Distribuição Global por FUNÇÃO (Sempre mostra o cenário macro)
            df_global = pd.DataFrame([{"Função": v.get("funcao", "Sem Função"), "Custo Mensal": v["custo_mensal"]} for v in st.session_state["db_funcionarios"].values()])
            df_agrupado = df_global.groupby("Função").sum().reset_index()
            
            fig2 = px.pie(df_agrupado, values='Custo Mensal', names='Função', hole=0.4, title="Custo MACRO por Função (Geral)")
            fig2.update_traces(textposition='inside', textinfo='percent+label')
            fig2.update_layout(showlegend=False)
            c_graf2.plotly_chart(fig2, use_container_width=True)

            st.divider()
            st.markdown("#### Gerenciar")
            c_del1, c_del2 = st.columns([2, 1])
            func_remover = c_del1.selectbox("Selecione um funcionário para remover:", options=list(st.session_state["db_funcionarios"].keys()))
            st.write("")
            if c_del2.button("🗑️ Excluir Funcionário e Recalcular"):
                del st.session_state["db_funcionarios"][func_remover]
                
                # Recalcula a folha e atualiza os custos fixos
                total_folha = sum(f["custo_mensal"] for f in st.session_state["db_funcionarios"].values())
                df_pessoal = st.session_state["df_custos_categorias"]["1. Despesa com pessoal"].copy()
                if "1.1 Total da folha de pagamento" in df_pessoal["ÍTEM"].values:
                    idx = df_pessoal.index[df_pessoal["ÍTEM"] == "1.1 Total da folha de pagamento"].tolist()[0]
            ...
            st.info("Nenhum funcionário cadastrado no sistema. Vá para a aba 'Cadastrar Funcionário'.")
...
elif modulo_selecionado == "6. Gestão de RH (Integração)": render_calculadora_rh()