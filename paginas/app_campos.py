...
try:
    ...
    st.error("⚠️ A biblioteca 'supabase' não está instalada. Abra o terminal e digite: pip install supabase")

# ==========================================
# 1. IDENTIFICAÇÃO DO CLIENTE / ARQUIVO
# ==========================================
# Identificador único deste cliente para o Supabase
ID_CLIENTE = "Bruno_Campos" 
...
# ==========================================
# MÓDULO 6: CALCULADORA DE CUSTO DE RH
# ==========================================
def render_calculadora_rh():
    st.title("Calculadora de Custo de RH (Empregador)")
    st.markdown("Descubra o **custo real** que um funcionário gera para a clínica, provisionando corretamente férias, 13º, benefícios e encargos fiscais mensais.")
    st.divider()
    ...
    vale_transporte = c4.number_input("Vale Transporte (R$)", min_value=0.0, step=10.0, value=0.0, help="Somente a parte paga pela empresa (descontando os 6% do funcionário).")
    ...
    st.markdown("💡 *Se a clínica for Simples Nacional (Anexo III), os encargos geralmente ficam perto de **8%** (apenas FGTS). Se for Lucro Presumido/Real, pode chegar a **~36% a 40%** (INSS Patronal, FGTS, RAT, Sistema S).*")
    encargos_pct = st.number_input("Alíquota Total de Encargos do Empregador (%)", min_value=0.0, max_value=100.0, step=1.0, value=8.0)

    # --- CÁLCULOS MATEMÁTICOS DE PROVISÃO ---
    ...
    st.divider()
    st.subheader("📊 Resumo do Custo Real do Funcionário")
    
    col_res1, col_res2 = st.columns(2)
    with col_res1:
        st.markdown(f"<div style='background-color: {COR_FUNDO_CLARO}; padding: 15px; border-radius: 5px;'>"
                    f"<h4 style='margin-top:0;'>CUSTO MENSAL PARA A CLÍNICA</h4>"
                    f"<h2 style='color: {COR_CABECALHO};'>R$ {custo_mensal:,.2f}</h2>"
                    f"</div>", unsafe_allow_html=True)
        st.write("")
        st.write(f"**1. Remuneração Direta:** R$ {remuneracao_total:,.2f}")
        st.write(f"**2. Provisão Férias (+1/3):** R$ {provisao_ferias:,.2f}")
        st.write(f"**3. Provisão 13º Salário:** R$ {provisao_13:,.2f}")
        st.write(f"**4. Encargos Sociais ({encargos_pct}%):** R$ {valor_encargos:,.2f}")
        st.write(f"**5. Benefícios Mensais:** R$ {beneficios_total:,.2f}")

    with col_res2:
        st.markdown(f"<div style='background-color: {COR_FUNDO_CLARO}; padding: 15px; border-radius: 5px;'>"
                    f"<h4 style='margin-top:0;'>IMPACTO TOTAL ANUAL</h4>"
                    f"<h2 style='color: {COR_CABECALHO};'>R$ {custo_anual:,.2f}</h2>"
                    f"</div>", unsafe_allow_html=True)
        
        df_grafico = pd.DataFrame({
            "Categoria": ["Salário/Comissões", "Provisões (Férias/13º)", "Encargos (Governo)", "Benefícios"],
            "Valor Mensal": [remuneracao_total, provisao_ferias + provisao_13, valor_encargos, beneficios_total]
        })
        
        fig = px.pie(df_grafico, values='Valor Mensal', names='Categoria', hole=0.4, title="Distribuição do Custo Mensal")
        fig.update_traces(textposition='inside', textinfo='percent+label')
        fig.update_layout(margin=dict(t=40, b=0, l=0, r=0), showlegend=False)
        st.plotly_chart(fig, use_container_width=True)

    st.info("💡 **Dica de Integração:** Pegue este **Custo Mensal** e jogue lá no módulo **2. Custos Fixos** (na categoria '1.1 Total da folha de pagamento'). Assim, o custo da sua Hora Clínica ficará perfeitamente ajustado à realidade!")
...
elif modulo_selecionado == "6. Calculadora de Custo de RH": render_calculadora_rh()